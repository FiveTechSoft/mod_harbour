procedure main()
   ...
   local cArgs := ap_getArgs(), nRecNo, nEdit

   USE ( ap_getScriptPath() + "data/customer.dbf" ) SHARED
   
   if ! Empty( cArgs )
      if Left( cArgs, Len( "next:" ) ) == "next:"
         nRecNo = Val( SubStr( cArgs, 6 ) )
         if nRecNo < RecCount() - 10
            GOTO nRecNo
         endif   
      endif
      if Left( cArgs, Len( "prev:" ) ) == "prev:"
         nRecNo = Val( SubStr( cArgs, 6 ) )
         if nRecNo > 20
            GOTO nRecNo - 10
         endif   
      endif
      if Left( cArgs, Len( "edit:" ) ) == "edit:"
         nRecNo = Val( hb_aTokens( cArgs, ":" )[ 3 ] )
         nEdit  = Val( hb_aTokens( cArgs, ":" )[ 2 ] )
         GOTO nRecNo
      endif      
   endif   
...
<div class="container-fluid">
          ...
          <tbody>
             <?prg local cRow := "", nRow := 1, n
                   while nRow < 11 .and. ! Eof()
                      cRow += '<tr>' 
                      for n = 1 to FCount()
                         cRow += '<td class="center">' + mh_valToStr( FieldGet( n ) ) + "</td>" + CRLF
                      next
                      cRow += "<td>" + CRLF
                      cRow += '<button type="button" class="btn btn-xs btn-default" onclick="location.href=' + ;
                              "'dbrowse2.prg?edit:" + ;
                                 hb_ntos( RecNo() ) + ":" + hb_ntos( RecNo() - nRow + 1 ) + "'" + '">'
    ...
    <div class="panel-footer">
       <div class="col-xs-3"><div class="dataTables_info" id="example_info">Showing <?prg return hb_ntos( RecNo() - 10 ) + " - " + hb_ntos( RecNo() - 1 ) ?> of <?prg return hb_ntos( RecCount() ) ?> total results</div></div>
          <div class="col-xs-6">
             <div class="dataTables_paginate paging_bootstrap">
                <ul class="pagination pagination-sm" style="margin:0 !important">
                   <li class="prev"><a href="dbrowse2.prg?prev:<?prg return hb_ntos( RecNo() - 10 ) ?>">Previous</a></li>
                   <li class="active"><a href="#">1</a></li>
                   <li><a href="#">2</a></li>
                   <li><a href="#">3</a></li>
                   <li><a href="#">4</a></li>
                   <li class="next"><a href="dbrowse2.prg?next:<?prg return hb_ntos( RecNo() ) ?>">Next</a></li></ul>
             </div>
          </div>
          <div class="btn-group">
   ...
   if nEdit != nil
      TEMPLATE
         ...
         <script>$( "#form" ).resizable().draggable();</script>'
         <script>
         function HideForm()
            {
