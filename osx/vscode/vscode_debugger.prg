...
static procedure sendStack()
	...
	for i:=start to d
		...
		IF ( p := AScan( aStack, {| a | a[ HB_DBG_CS_LEVEL ] == l } ) ) > 0
			...
			line			:= procLine(i)
			module			:= If( "{SOURCE}" $ procFile(i), AP_FileName(), procFile(i) )
			functionName	:= If( "{SOURCE}" $ ProcName(i), AP_FileName(), ProcName(i) )
...
static function inBreakpoint()
	...
	local idLine, cFile := lower(If( "{SOURCE}" $ procFile(3), AP_FileName(), procFile(3 ) ) ), nExtra := 2
	LOCAL ck
   ...
   QOut( hb_ArgV( 0 ) )
	switch nMode
		case HB_DBG_MODULENAME
			...
			if i=0
				tmp[HB_DBG_CS_MODULE] := If( "{SOURCE}" $ uParam1, AP_FileName(), uParam1 )
				tmp[HB_DBG_CS_FUNCTION] := procName(1) 
			else
				tmp[HB_DBG_CS_MODULE] := If( "{SOURCE}" $ left(uParam1,i-1),;
											 SubStr( AP_FileName(), At( ":", AP_FileName() ) + 1 ),;
											 left(uParam1,i-1) ) 
			...
			if at("_INITSTATICS", tmp[HB_DBG_CS_FUNCTION])<>0
				tmp[HB_DBG_CS_MODULE] := If( "{SOURCE}" $ ProcFile(1), AP_FileName(), ProcFile( 1 ) )
